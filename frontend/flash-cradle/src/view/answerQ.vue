<template>
    <div class="container">
        ...
        <div class="submit" v-if="statu == 0 && abandon == undefined">
            <span style="position: absolute; left: 50%; top:15px;transform: translate(-50%, 0); 
            font-size: 20px; font-weight: 600; color: #0095ff;">截止时间：</span>
            <el-countdown title="" :value="end_time" v-if="have_time" @finish="hilarity1" style="position: absolute; left: 50%; top:40px;transform: translate(-50%, 0); 
            font-size: 18px; font-weight: 600; color: #0095ff;" value-style="font-size: 28px; font-weight: 600;" />
            <span style="position: absolute; left: 50%; top:50px;transform: translate(-50%, 0); 
            font-size: 20px; font-weight: 600; color: #0095ff;" v-if="!have_time">不限时</span>
    ...
    <div v-if="statu == 3">
        ...
        <div v-if="type == 2" class="toufen">
            <div class="question" v-for="(q, index) in list" :key="index">
                <div class="description" v-if="q.img_url">{{index + 1}}.{{q.description}}</div>
                <div v-if="q.img_url">
...
import { getUserId, getAnsweredQ, setAnsweredQ, getUserMoney, setUserMoney,imgUrl } from "../../http/cookies";
    ...
    methods: {
                ...
                .then((res) => {
                    ...
                    if (res.data.status_code == 1) {
                        ...
                        ElMessageBox.alert('问卷填写成功', 'Success!', {
                            ...
                            callback: () => {
                                this.statu = 3;
                                setUserMoney(1+ parseInt(getUserMoney()));
                                if (this.type == 1) {
                                    this.computeScore();
                                }
        ...
        getAlterArray() {
            ...
            for(let i = 0;i<this.randomArray.length;i++){
                this.alterArray.push(0);
            }
            for(let i = 0;i<this.randomArray.length;i++){
                this.alterArray[parseInt(this.randomArray[i])] = i;
            }
        },
        error_lookup() {
            if (this.statu != 0) {
            ...
            this.error_message = ''
            console.log("this.warning ", this.list);
            console.log("this.warning ", this.warning);
            console.log("this.warning ", this.alterArray);
            for (let i = 0; i < this.warning.length; i++) {
                if (this.warning[this.alterArray[i]] != '') {
                    if(this.warning[this.alterArray[i]] == '1'){
                        this.error_message = '请做答第' + (this.alterArray[i] + 1) + '题';
                    }
                    else if(this.warning[this.alterArray[i]] == '2'){
                        this.error_message = '第' + (this.alterArray[i] + 1) + '题超出字数限制(256)';
                    }
                    else{
                        this.error_message = '请做答第' + (this.alterArray[i] + 1) + '题';
                    }
        ...
        tryGetAnswer() {
            ...
            if(content!=undefined && this.abandon==undefined && content!=''){
