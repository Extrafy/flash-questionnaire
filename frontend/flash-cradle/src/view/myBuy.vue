...
<script>
import SurveyBlock from "../components/survey.vue";
    ...
    handleGetQ() {
      ...
      getQ(data)
        .then((res) => {
          ...
          console.log(res.data.list4);

          // 合并 list1 和 list4 并去重
          const list1 = res.data.list1;
          const list4 = res.data.list4;
          const combinedList = [...list1, ...list4];

          // 使用 Map 去重，假设 id 是唯一标识
          const map = new Map();
          combinedList.forEach((item) => {
            map.set(item.id, item);
          });
          this.blocks = Array.from(map.values());

          this.purchased = res.data.list2;
          this.collectedList = res.data.list3;

          // 将已收藏问卷的 iscollected 设置为 true
          this.collectedList.forEach((collectedItem) => {
            const block = this.blocks.find(
              (block) => block.id === collectedItem.id
            );
            if (block) {
