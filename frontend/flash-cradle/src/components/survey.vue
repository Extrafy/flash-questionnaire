<template>
    <div class="wrapper" @mouseenter="isHovering = true" @mouseleave="isHovering = false" :style="blockStyle">
      ...
      <template #footer>
        <div class="dialog-footer">
          ...
          <el-button type="primary" :disabled="leftMoney<3" @click="handlePurchase()">
            确定
          </el-button>
        </div>
      </template>
    </el-dialog>
        <div class="title">{{ title.length > 7 ? title.substring(0, 7) + "..." : title }}
            <el-icon v-if="isCollected" :size="20" class="rate-theme-item-option" @click="handleUnCollect()" color="#409eff">
                    <StarFilled />
            </el-icon>
            <el-icon v-if="!isCollected" :size="20" class="rate-theme-item-option" @click="handleCollect()" :color="collectedColor">
                    <Star />
            </el-icon>
        </div>
        ...
        <div class="content">
            ...
            <div class="content-word" v-if="!isHovering">问卷状态：{{ state_list[state] }}</div>
            <div class="content-word" v-if="!isHovering">参加人数：{{ used_time }}人</div>
            <div class="content-word" v-if="!isHovering">问卷类型：{{ type_list[type] }}</div>
            <div v-if="isHovering">
                <button class="content-button" @click="prePurchase()">浏览</button>
...
export default {
    ...
    data() {
        return {
            ...
            isHovering: false,
            state_list:['已停止','已发布', '未知'],
            type_list:['调查', '考试', '投票', '报名'],
        };
    },
    computed: {
        isCollected() {
    ...
    methods: {
        ...
        prePurchase() {
            console.log(this.ownedList);
            let MaxLength = Math.max(this.ownedList.length,this.purchased.length);
            for(let i = 0; i < MaxLength; ++i) {
                if((i<this.purchased.length && this.purchased[i].id == this.id)|| (i< this.ownedList.length &&this.ownedList[i].id == this.id) ) {
                    //去预览fuckman
        ...
        handleWriteQ() {
            ...
            this.$router.push({path: '/answer',query:{q_id: this.id, reward: true,}});
        },
    },
    mounted() {
